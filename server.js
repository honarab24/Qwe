...
app.get('/extract', async (req, res) => {
  ...
  try {
    ...
    // Replace stream URLs with proxied versions (so streams also go through us)
    const proxiedText = text.replace(
      /(https?:\/\/[^\s]+)/g,
      (url) => `${req.protocol}://${req.get('host')}/proxy?url=${encodeURIComponent(url)}&ua=${encodeURIComponent(ua)}`
    );
...
// Proxy route for streams (TS, HLS, etc.)
